...
class SpecificationUtilsTest {
    ...
    void getWorkspacesSpecification_ShouldCreateSpecificationSuccessfully() {
        ...
        Join<Workspace, UserWorkspace> userWorkspaceJoin = mock(Join.class);
        Join<UserWorkspace, User> userJoin = mock(Join.class);
        jakarta.persistence.criteria.Predicate userCondition = mock(jakarta.persistence.criteria.Predicate.class);

        when(root.<Workspace, UserWorkspace>join("userWorkspaces")).thenReturn(userWorkspaceJoin);
        when(userWorkspaceJoin.<UserWorkspace, User>join("user")).thenReturn(userJoin);
        ...
        verify(root).join("userWorkspaces");
        verify(userWorkspaceJoin).join("user");
